...
PydanticAI) implementing Milestone 1 — Google sign-in, customers, visits,
...
```bash
cp .env.example .env   # fill in DASHSCOPE_API_KEY for real AI drafting; other defaults work
docker compose up -d   # Postgres 17, Redis 8, MinIO (+ bucket init)
uv sync --dev
