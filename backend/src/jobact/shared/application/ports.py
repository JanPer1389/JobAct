...
class ObjectStorage(Protocol):
    """S3/MinIO-shaped object storage.

    Client uploads use presigned URLs and are verified with ``head()``.
    Server-side workflows may also download evidence and upload generated
    artifacts such as signed PDFs.
    """
    ...
    async def download(self, key: str) -> bytes:
        """Return bytes for a server-side generated or consumed object."""
        ...

    async def upload(self, key: str, data: bytes, content_type: str) -> ObjectMetadata:
        """Store server-generated bytes and return their verified metadata."""
        ...
